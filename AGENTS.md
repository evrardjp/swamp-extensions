...
<!-- inspired by evrardjp's version. Feel free to comment on this and help me! -->
...
- `models/`, `vaults/`, `datastores/`, `reports/` - Implementation files
...
```yaml
manifestVersion: 1
name: "@evrardjp/extension-name"
version: "2026.07.03.1"          # CalVer: YYYY.MM.DD.N
...
dependencies:
  - "@evrardjp/other-ext@2026.04.12.1"
...
- Extension names: `@evrardjp/<name>` (e.g., `@evrardjp/ssh-ca`) or `@evrardjp/<category>/<name>` for grouped extensions.
...
Run from extension directory (e.g., `cd ssh-ca/`):
...
Project-level skills in `.agents/skills/` at the repo root:

- `pr-workflow.md` - End-to-end PR workflow: branch, develop, push, respond to adversarial review, iterate until CI passes, merge
- `update-stale-deps.md` - Fix stale swamp extension dependency pins in workflow manifests
- `bump-npm-deps.md` - Audit and upgrade npm dependencies (AWS SDK, zod, postgres) across all extensions
