...
```bash
swamp model create @evrardjp/github-project-activity eso-external-secrets-activity \
...
`sync_github_repo --input includeForkIndex=true` refreshes repository metadata and
indexes fork repositories that may appear as PR heads. The fork index is metadata
only: it lets reports and agents recognize configured/discovered forks without
turning each fork into a full activity ingestion target.
...
fork needs full tracking, create a separate `@evrardjp/github-project-activity`
...
`@evrardjp/github-project-briefing` is a model-scope report over stored Swamp data.
