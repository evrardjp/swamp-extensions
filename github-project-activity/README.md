...
2. **Activity events** — chronological facts from GitHub and
   private/manual/agent sources for timelines and historical reports.

## Model and report types

- Model: `@evrardjp/github-project-activity`
- Report: `@evrardjp/github-project-briefing`

Recommended pattern: create a separate model instance for each project/repo:

```bash
swamp model create @evrardjp/github-project-activity external-secrets-external-secrets-project-activity \
...
For public repositories, a fine-grained token with read-only repository access
is enough for most ingestion:
...
```bash
swamp model method run external-secrets-external-secrets-project-activity sync_github_repo --input includeForkIndex=true
swamp model method run external-secrets-external-secrets-project-activity sync_github_fork_index
swamp model method run external-secrets-external-secrets-project-activity sync_github_recent_activity
swamp model method run external-secrets-external-secrets-project-activity sync_github_prs --input state=open
swamp model method run external-secrets-external-secrets-project-activity sync_github_issues --input state=open
swamp model method run external-secrets-external-secrets-project-activity sync_github_backfill --input since=2026-01-01 --input until=2026-07-01
```

`sync_github_repo --input includeForkIndex=true` refreshes repository metadata
and indexes fork repositories that may appear as PR heads. The fork index is
metadata only: it lets reports and agents recognize configured/discovered forks
without turning each fork into a full activity ingestion target.
...
# GitHub labels.
swamp model method run external-secrets-external-secrets-project-activity classify_stale_candidates
...
swamp model method run external-secrets-external-secrets-project-activity clear_stale_candidates
```
...
will not remove it. Add a future GitHub-label mutation method, with
write-capable token scopes, to keep GitHub labels in sync.
...
version: 1
tags:
  domain: github-project-activity
  cadence: daily
trigger:
  schedule: "0 6 * * *"
reports:
  ...
  - name: classify-stale
    ...
    steps:
      - name: sync-recent-activity
        task:
          type: model_method
          modelIdOrName: external-secrets-external-secrets-project-activity
...
```bash
swamp model method run external-secrets-external-secrets-project-activity record_activity --stdin
swamp model method run external-secrets-external-secrets-project-activity record_artifact --stdin
```

Use `record_activity` when project-relevant context does not exist in GitHub or
should be stored as private/operator context: manual maintainer notes, agent
session outcomes, triage decisions, follow-up reminders, release-blocker notes,
or links to large artifacts stored with `record_artifact`. The method appends an
`activityEvent` to the chronological ledger and defaults missing repo,
visibility, source, actor, timestamp, tags, and artifact references from the
model configuration.

Render a stored-data-only PR dossier:

`render_pr_report` is intentionally a model method, not only a Swamp report: it
needs runtime inputs (`prNumber`, `includePrivate`, optional `since`/`until`),
writes a durable `prReport` data object named for that PR, and is used as an
explicit “render this PR now” action rather than a passive post-run execution
summary.

```bash
swamp model method run external-secrets-external-secrets-project-activity render_pr_report \
  --input prNumber=6530 \
  --input includePrivate=true
swamp data get external-secrets-external-secrets-project-activity pr-report-external-secrets-external-secrets-6530 --json
```

For a report-shaped view, use `@evrardjp/pr-review`. It prefers an existing
`prReport`; if none exists for the selected PR, it uses the same stored-data
rendering structure as `render_pr_report` and returns that generated review
markdown (persisting `prReport` too when the report context supports model
resource writes).

```bash
swamp report get @evrardjp/pr-review \
  --model external-secrets-external-secrets-project-activity \
  --markdown
```

## Fork model

`sync_github_fork_index` records fork metadata in the current model only. It
does **not** ingest full fork issue/PR/comment/review/file activity. If an
important fork needs full tracking, create a separate
`@evrardjp/github-project-activity` model instance with `owner`/`repo` set to
that fork.
...
`@evrardjp/github-project-briefing` is a model-scope report over stored Swamp
data. It does not fetch GitHub itself.

```bash
swamp report get @evrardjp/github-project-briefing --model github-project-activity --markdown
swamp report get @evrardjp/github-project-briefing --model github-project-activity --json
```
...
It tells agents how to fetch `@evrardjp/github-project-briefing`, drill into a
PR/issue, and record durable conclusions through `record_activity` and
`record_artifact`.

## Fixture ingestion and benchmarking

For ingestion benchmarks isolated from the live GitHub API, use the fixture
split:

```bash
# Fetch once from GitHub and store raw/minimally processed API responses.
swamp model method run github-project-activity fetch_from_github --stdin <<'JSON'
{"scenario":"eso-2026-07-window","sourceMethod":"sync_github_backfill","methodArgs":{"since":"2026-06-01T00:00:00Z","until":"2026-07-01T00:00:00Z","state":"all"}}
JSON

# Replay the stored responses, no GitHub API calls, and write derived resources.
swamp model method run github-project-activity ingest_fetched_data --stdin <<'JSON'
{"scenario":"eso-2026-07-window","sourceMethod":"sync_github_backfill","methodArgs":{"since":"2026-06-01T00:00:00Z","until":"2026-07-01T00:00:00Z","state":"all"}}
JSON
```

`fetch_from_github` writes `rawGithubApiResponse` resources.
`ingest_fetched_data` replays those fixtures through the same ingestion paths
used by the existing sync methods, so benchmarks include Swamp method runtime
and Swamp write cost.

A separate benchmark-result model type is included:

```bash
swamp model create @evrardjp/github-project-activity-benchmark github-project-activity-benchmarks
```

The external harness lives at `scripts/bench_github_project_activity.ts` and can
record observations in that separate model:

```bash
deno run --allow-run --allow-env scripts/bench_github_project_activity.ts \
  --mode swamp-ingest \
  --model github-project-activity \
  --scenario eso-2026-07-window \
...
Use `--mode external --external-command '<your db/no-swamp ingestion command>'`
to time a database or no-Swamp baseline and store it in the same benchmark
ledger.

## Development checks

```bash
deno check models/github_project_activity.ts reports/github_project_briefing.ts
deno test models/github_project_activity_test.ts reports/github_project_briefing_test.ts
...
```bash
swamp model method run external-secrets-external-secrets-project-activity sync_github_repo \
...
```bash
swamp model method run external-secrets-external-secrets-project-activity sync_github_prs \
    --input state=open \
    --input limit=3 \
...
(Optional): Sync the rest of the PRs, issues, etc. swamp model method run
external-secrets-external-secrets-project-activity sync_github_prs\
--input state=open\
--input limit=20\
--input includeReviews=true\
--input includeReviewComments=true\
...
swamp model method run external-secrets-external-secrets-project-activity
sync_github_issues\
--input state=open\
--input limit=20\
...
```bash
swamp model method run external-secrets-external-secrets-project-activity sync_github_backfill \
  --input since=2026-06-01 \
  --input until=2026-07-02 \
  --input limit=100
```

5. Classify stale candidates and clear stale classifications for recently active
   items:
...
```bash
swamp report get @evrardjp/github-project-briefing \
    --model external-secrets-external-secrets-project-activity \
    --markdown
```
...
```bash
swamp data query 'modelName == "external-secrets-external-secrets-project-activity" && specName == "prSnapshot"' \
    --select '{"number": attributes.number, "title": attributes.title, "url": attributes.url}'

  swamp model method run external-secrets-external-secrets-project-activity render_pr_report \
    --input prNumber=<PR_NUMBER>

  swamp data get external-secrets-external-secrets-project-activity pr-report-external-secrets-external-secrets-<PR_NUMBER> --json \
...
```bash
swamp data get external-secrets-external-secrets-project-activity \
...
swamp data query 'modelName == "external-secrets-external-secrets-project-activity" && specName == "activityEvent" && attributes.subjectType == "pr" && attributes.subjectNumber == 6546' \
...
swamp data query 'modelName == "external-secrets-external-secrets-project-activity" && specName == "prFileSnapshot" && attributes.prNumber == 6546' \
...
swamp data query 'modelName == "external-secrets-external-secrets-project-activity" && specName == "ciStatusSnapshot" && attributes.prNumber == 6546' \
